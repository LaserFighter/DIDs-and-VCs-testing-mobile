...
async fn main() -> anyhow::Result<()> {
	...
	let mut secret_manager_issuer: SecretManager =
	SecretManager::Stronghold(
		StrongholdSecretManager::builder()
		  .password("secure_password_1")
		  .build(random_stronghold_path())?,
	);
	let (_, issuer_document, key_pair_issuer): (Address, IotaDocument,
	KeyPair) =
	...
	// Decentralize the presentation from the holder:
	...
	let presentation_verifier_options: VerifierOptions =
	VerifierOptions::new()
	...
	PresentationValidationOptions::default()
	  .presentation_verifier_options(presentation_verifier_options).shared_validation_options(credential_validation_options).subject_holder_relationship(SubjectHolderRelationship::AlwaysSubject);
